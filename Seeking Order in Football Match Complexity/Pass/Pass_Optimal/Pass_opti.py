# -*- coding: utf-8 -*-
"""
Pass L / E' · DML-informed independent multi-KPI GA policy optimization
========================================================================
...
This Pass version retains the leakage-safe feature allow-list and integer decision constraints, and adds the reviewer-facing robustness analyses required for the independent policy stage: a context-only predictive baseline, alternative policy learners, both global and within-match conditional label-permutation diagnostics, explicit optimal-strategy value tables, and GA random-seed stability. It does not pass every
...
Workflow
--------
1. By default run both L and E' sequentially; ``--stage L`` or
   ``--stage Eprime`` can be used for a single-stage rerun. Read the completed
   Pass single-KPI and joint-DML workbooks for each stage.
2. Cross-check them and build a cluster-specific candidate pool containing only KPIs that passed both
...
3. Select one KPI per prespecified tactical family for the main analysis:
   ...
   The default rule is the largest absolute joint effect per 1 SD among eligible candidates.
4. End the DML stage.
...
Examples
--------
Run both stages automatically (default)::

    python pass_L和Eprime_自动双阶段_独立预测多KPI_GA策略.py

Strict E' analysis::

    python pass_L和Eprime_自动双阶段_独立预测多KPI_GA策略.py --stage Eprime

Strict L analysis (stops if a tactical family has no fully eligible KPI)::

    python pass_L和Eprime_自动双阶段_独立预测多KPI_GA策略.py --stage L --l-selection strict

Exploratory L analysis using the three-KPI combinations in the manuscript
table, while preserving failed-gate flags in the selection audit::

    python pass_L和Eprime_自动双阶段_独立预测多KPI_GA策略.py --stage L
"""
...
RAW_INPUT_PATH = (
    r"D:\pipeline\返修\聚类\传球cluster_sensitivity_K2_K5_仅修复end_location\Pass_K3_main_for_DML_仅修复end_location.xlsx"
)
SINGLE_RESULTS_XLSX = (
    r"D:\pipeline\返修\DML部分\pass_dml_单指标_L和Eprime_RF主模型_含训练fold内下采样敏感性.xlsx"
)
ROTATING_JOINT_RESULTS_XLSX = (
    r"D:\pipeline\返修\DML部分\pass_dml_L和Eprime_全部通过KPI_轮换联合交互DML_含训练fold内下采样敏感性.xlsx"
)
OUTPUT_XLSX = (
    r"D:\pipeline\返修\DML部分\pass_Eprime_DML筛选后_独立预测模型_多KPI联合GA策略_v1最终稳健性.xlsx"
)
OUTPUT_XLSX_L = (
    r"D:\pipeline\返修\DML部分\pass_L_DML筛选后_独立预测模型_多KPI联合GA策略_v1最终稳健性.xlsx"
)

PIPELINE_VERSION = "pass_L_Eprime_independent_policy_v1_4_memory_safe_2026-07-31"
STAGE = "E'"
RANDOM_SEED = 42
...
# The L combinations requested for the compact manuscript-style table.
# These are NOT all strictly eligible under the original joint-DML gates:
# the script therefore activates them only with ``--l-selection table-override``
# and records every failed gate in 01_main_KPI_selection.
L_TABLE_KPI_OVERRIDE: Dict[int, Tuple[str, str, str]] = {
    0: ("Adv_5(L)", "Avg_1_Def(L)", "Spr_Def(L)"),
    1: ("Adv_5(L)", "Avg_1_Def(L)", "Area_Def(L)"),
    2: ("Adv_5(L)", "Avg_1_Def(L)", "Spr_Def(L)"),
}
...
FORBIDDEN_FEATURE_TOKENS = (
    ...
    "pass_outcome", "ball_receipt_outcome", "end_reason", "defensive_success",
)
...
# The Pass policy may select either Adv_5 or Adv_10; both are count-valued and must remain integers.
...
def _single_pass_flag(frame: pd.DataFrame) -> pd.Series:
    """Use the strictest available single-KPI screening flag."""
    if "candidate_for_discussion" in frame.columns:
        return frame["candidate_for_discussion"].map(as_bool)
    if "reviewer_primary" in frame.columns:
        return frame["reviewer_primary"].map(as_bool)
    if "reviewer_evidence_status" in frame.columns:
        return frame["reviewer_evidence_status"].astype(str).str.upper().eq("PRIMARY")
    if "q_global" in frame.columns:
        return pd.to_numeric(frame["q_global"], errors="coerce").le(0.05)
    raise ValueError(
...
def canonical_stage_label(value: Any) -> str:
    """Normalize CLI/workbook stage labels to ``L`` or ``E'``."""
...
def _normalize_joint_results(path: str) -> pd.DataFrame:
    ...
    if "02_target_AME_report" in sheets:
        ...
        if missing:
            raise ValueError(
                "Carry-style rotating joint workbook is missing required columns: "
                + " | ".join(missing)
            )
        out = joint.copy()
        out["joint_schema"] = "rotating_AME_report"
        out["joint_effect_per_1sd"] = pd.to_numeric(out["estimate"], errors="coerce")
        out["joint_q_global"] = pd.to_numeric(out.get("q_global"), errors="coerce")
        ...
        for col in (
            ...
            if col in out.columns:
                gate_parts.append(out[col].map(as_bool))
        if not gate_parts:
            raise ValueError(
                "02_target_AME_report does not contain recognized joint-screening gates."
            )
        joint_pass = gate_parts[0].copy()
        for part in gate_parts[1:]:
            joint_pass &= part
        out["joint_stage_pass"] = joint_pass
        ...
        out["joint_robustness_gate_source"] = (
            "single pass + global FDR + direction + under-sampling/optimizer gate"
        )
        ...
        if missing:
            raise ValueError(
                "Pass-style joint workbook is missing required columns: "
                + " | ".join(missing)
            )
        out = joint.copy()
...
def load_candidate_pool(single_path: str, joint_path: str) -> pd.DataFrame:
    """
    Cross-check the selected Pass stage against BOTH completed DML analyses.
    ...
    existing Pass unified-family joint-selection workbook.
    """
        ...
        if cluster in FINAL_KPI_OVERRIDE:
            ...
            for family, kpi in zip(EXPECTED_FAMILIES, kpis):
                ...
                if actual_family != family:
                    raise ValueError(
                        f"Manual override family mismatch for Cluster {cluster}, {kpi}: "
            ...
            if noneligible and not ALLOW_NONELIGIBLE_OVERRIDE:
                raise RuntimeError(
                    ...
                    "For the explicitly exploratory L table analysis, rerun with "
                    "--stage L --l-selection table-override."
                )
            if noneligible:
                warnings.warn(
                    f"Exploratory L override in Cluster {cluster}: "
...
DEFENSIVE_STAGE_PATTERN = re.compile(
    r"^(Adv_\d+|Avg_\d+_Def|Area_Def|Spr_Def|DistToDefCentroid)\((?:L|E')\)$",
    flags=re.IGNORECASE,
)


def _read_raw_table(path: str) -> Tuple[pd.DataFrame, str]:
    """Read a raw Pass table from CSV or Excel and return (frame, source_sheet)."""
...
def build_feature_columns(
    ...
    if missing:
        raise ValueError(
            ...
            f"Check that the raw CSV/Excel worksheet contains the full Pass {STAGE} table."
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--stage",
        choices=("both", "L", "Eprime"),
        default="both",
        help=(
            "both (default) runs L and E' sequentially and writes two workbooks; "
            "L or Eprime runs only that stage."
        ),
    )
    parser.add_argument(
        "--l-selection",
        choices=("strict", "table-override"),
        default="table-override",
        help=(
            "For --stage L/both: table-override (default) uses the manuscript-table "
            "three-KPI combinations and marks failed gates as exploratory in the audit; "
            "strict stops if any tactical family has no fully eligible KPI."
        ),
    )
    parser.add_argument(
        "--output",
        default=None,
        help=(
            "Output workbook for a single-stage run. Do not use with --stage both; "
            "use --output-l and --output-eprime instead."
        ),
    )
    parser.add_argument(
        "--output-l",
        default=None,
        help="Optional L output workbook when --stage both is used.",
    )
    parser.add_argument(
        "--output-eprime",
        default=None,
        help="Optional Eprime output workbook when --stage both is used.",
    )
...
def build_stage_command(args: argparse.Namespace, stage: str) -> List[str]:
    """Build one child-process command for an isolated stage run."""
    if stage not in {"L", "Eprime"}:
    ...
    command = [
        ...
        "--stage", stage,
        "--raw", str(args.raw),
        "--single-results", str(args.single_results),
        "--joint-results", str(args.joint_results),
        "--l-selection", args.l_selection if stage == "L" else "strict",
    ]

    stage_output = args.output_l if stage == "L" else args.output_eprime
    if stage_output:
        command.extend(["--output", str(stage_output)])
    if args.global_permutation_reps is not None:
        command.extend([
            "--global-permutation-reps",
            str(args.global_permutation_reps),
        ])
    if args.conditional_permutation_reps is not None:
        command.extend([
            "--conditional-permutation-reps",
            str(args.conditional_permutation_reps),
        ])
    if args.quick_smoke:
...
def run_both_stages(args: argparse.Namespace) -> None:
    """Run L and E' in isolated child processes so globals cannot leak."""
    if args.output is not None:
        raise ValueError(
            "--output is ambiguous with --stage both. "
            "Use --output-l and --output-eprime."
        )

    print("=" * 88)
    print("Pass automatic dual-stage policy optimization")
    print("The script will run L first, then E', and write two separate workbooks.")
    print(f"L selection mode: {args.l_selection}")
    print("=" * 88)

    for stage in ("L", "Eprime"):
        display = "L" if stage == "L" else "E'"
        print(f"[DUAL-STAGE] Starting {display} ...", flush=True)
        subprocess.run(build_stage_command(args, stage), check=True)
        print(f"[DUAL-STAGE] Completed {display}.", flush=True)

    print("=" * 88)
    print("Both L and E' policy optimizations completed.")
    print(f"L output : {args.output_l or OUTPUT_XLSX_L}")
    print(f"E' output: {args.output_eprime or OUTPUT_XLSX}")
    print("=" * 88)


def configure_stage(args: argparse.Namespace) -> None:
    """Set the stage-specific globals before any workbook is loaded."""
    global STAGE, FINAL_KPI_OVERRIDE
    global ALLOW_NONELIGIBLE_OVERRIDE, ACTIVE_SELECTION_RULE

    STAGE = "L" if args.stage == "L" else "E'"

    if STAGE == "L" and args.l_selection == "table-override":
        FINAL_KPI_OVERRIDE = dict(L_TABLE_KPI_OVERRIDE)
        ALLOW_NONELIGIBLE_OVERRIDE = True
        ACTIVE_SELECTION_RULE = "L_manuscript_table_override_exploratory"
    else:
        FINAL_KPI_OVERRIDE = {}
        ALLOW_NONELIGIBLE_OVERRIDE = False
        ACTIVE_SELECTION_RULE = SELECTION_MODE
    ...
    if args.output is None:
        stage_specific_output = (
            args.output_l if STAGE == "L" else args.output_eprime
        )
        args.output = (
            stage_specific_output
            if stage_specific_output
            else (OUTPUT_XLSX_L if STAGE == "L" else OUTPUT_XLSX)
        )
...
def main():
    ...
    args = parse_args()
    if args.stage == "both":
        run_both_stages(args)
        return
    configure_stage(args)
    if args.quick_smoke:
        apply_quick_smoke()
    if args.global_permutation_reps is not None:
    ...
    started = time.time()
    print("=" * 88)
    print("Pass L/E' independent DML-informed multi-KPI GA policy optimization (v1.4 memory-safe)")
    print(f"Stage          : {STAGE}")

...
PASS_FEATURES_PATH = r""
PASS_FEATURES_SHEET = 0
...
# 并行
MAX_WORKERS = 3  # 你有几个 cluster 就开几个；CPU 紧张可改 2
...
L_cluster_0\tAvg_1_Def(L)\t-0.123713952\t-0.09\t-10.2479222\t0.009156035\t11607\t0.222193504\t0.43769374\t9.67318E-24\t2.75694E-15
E_cluster_0\tArea_Def(E)\t-0.067341975\t-0.054659585\t8.133942726\t0.006187807\t10853\t0.222979821\t0.3931518\t1.6622E-15\t6.13057E-05
E_cluster_0\tSpr_Def(E)\t0.106521691\t0.095150798\t-7.390316474\t0.005113635\t10853\t0.222979821\t0.471747738\t3.90613E-13\t4.84998E-07
L_cluster_1\tAvg_1_Def(L)\t-0.119193995\t-0.094928955\t-10.55610779\t0.009596699\t11744\t0.229223433\t0.430741822\t3.80793E-25\t6.17683E-18
E_cluster_1\tAdv_10(E)\t0.044701204\t0.047160791\t10.22196668\t0.009563746\t11052\t0.229732175\t0.689220343\t1.2648E-23\t9.89534E-13
E_cluster_1\tArea_Def(E)\t-0.064096036\t-0.080805756\t-6.96470798\t0.004462682\t11052\t0.229732175\t0.415524383\t8.77562E-12\t7.38724E-11
L_cluster_2\tAdv_5(L)\t-0.057090266\t-0.037212502\t-8.126994387\t0.005931545\t11305\t0.18089341\t0.251001613\t1.76026E-15\t3.01508E-07
E_cluster_2\tAdv_5(E)\t-0.026003915\t-0.028528422\t-4.221817657\t0.001698822\t10697\t0.174908853\t0.36329163\t3.87744E-05\t3.09954E-05
E_cluster_2\tAvg_1_Def(E)\t-0.081451169\t-0.080665199\t-6.973005522\t0.004620788\t10697\t0.174908853\t0.675242213\t2.48192E-11\t8.04568E-10
"""
...
def load_pass_features(path, sheet=0):
...
def choose_model_features(sub_all, def_kpis, att_kpis):
    ...
    提速版：只保留
    1) 进入策略搜索的 KPI
    2) 少量背景数值变量（剔除 L/E KPI 以外的原始显式 meta / object / 嵌套字段）
    """
...
def process_one_cluster(stage, cid, sub_all, robust_df_full):
...
def run_for_stage_parallel(stage, df_all, cluster_to_results, writer):
    stage = stage.upper()
    print(f"\n========== 并行计算阶段 {stage} ==========")
    ...
    for r in ok_results:
        ...
        sheet_name = f"{stage}_cluster_{cid}_policy"[:31]
    ...
    if summary_rows:
        ...
        df_summary.to_excel(writer, index=False, sheet_name=f"summary_{stage}")
...
def main():
    ...
    df_all = load_pass_features(PASS_FEATURES_PATH, PASS_FEATURES_SHEET)
    ...
    with pd.ExcelWriter(OUTPUT_POLICY_XLSX, engine="xlsxwriter") as writer:
        for stage in STAGES:
            run_for_stage_parallel(stage, df_all, cluster_to_results, writer)

# -*- coding: utf-8 -*-
"""
Shot · All single-stage-passed KPI rotating joint interaction DML · L only
========================================================================
...
through a three-mechanism joint interaction model at the shot-location L stage.
For each target KPI, the target replaces the anchor in its own mechanism family,
while the best screened KPI from the other two families is retained as a companion
anchor. If a family has no strictly passed target, its smallest-q KPI is used only
as a reference companion and is never promoted to a passed target. Identical three-KPI
...
RANDOM_SEED = 42
N_SPLITS = 5
ACTIVE_STAGES = ("L",)  # Shot only has the shot-location L stage
...
# Compatibility switch retained from the Carry template. The Shot input table is
# already event-filtered, so no Dribble-row exclusion is applied.
EXCLUDE_DRIBBLE_ROWS = False
TYPE_NAME_COL = "event_type_name"
EXCLUDED_TYPE_NAMES = {"dribble"}

# Shot uses L-stage attacking KPIs only. Endpoint-stage KPIs remain in the raw
# table for audit but never enter either D or X.
...
# Exact reference copied from the final Shot single-KPI main model.
...
# Example: when D = Avg_1_Def(L), all Avg_Att(L) and
# DistToAttCentroid(L) controls are excluded from X.
EXCLUDE_SAME_SERIES_ATT_CONTROLS = True

# Shot has no endpoint-stage covariate block.
EXCLUDE_SAME_SERIES_ATT_CROSS_STAGE = False
...
# The active Shot stage suffix is (L).
...
# All score variables use the state BEFORE the current shot.
# Deliberately excluded from the main-model X:
...
# Therefore the nuisance models do not condition on exact shot location/geometry,
...
# Retained only for compatibility with shared helper functions; E' is never run.
...
# Legacy configuration retained for unused earlier entry points. The active final
# entry point selects anchors from the Shot single-KPI workbook.
JOINT_ANCHOR_BY_CLUSTER_STAGE = {
    (cluster, stage): {
        "local_advantage": f"Adv_5({stage})",
        "distance_centroid": f"Avg_1_Def({stage})",
        "structure_shape": f"Spr_Def({stage})",
    }
    for cluster in (0, 1, 2)
    for stage in ACTIVE_STAGES
}

# The joint script assumes these eight candidates have already entered the joint stage.
# It deliberately does not read a single-KPI workbook.
ASSUME_ALL_BUILTIN_CANDIDATES_PASSED_SINGLE_STAGE = True
...
def validate_primary_rf_alignment() -> None:
    ...
    if mismatches:
        raise ValueError(
            "Primary RF settings no longer match the final Shot L-only single-KPI model: "
        ...
        for base in bases:
            ...
            if missing:
                target_map.append({
                    ...
                    "single_model_pass_assumed": True,
                    "selection_basis": (
...
def build_target_dataset(
    ...
    if stage == "L":
        ...
        estimand = (
            ...
            "shot endpoint in the L-time freeze frame, given reduced pre-event "
            "background and permitted Att(L)/Att(E') controls; the target-matched "
            "attacking series is removed from both stages; when D is Avg_Def, "
            "all Avg_Att and DistToAttCentroid controls are removed; when D is "
            "DistToDefCentroid, DistToAttCentroid and all Avg_Att controls are "
...
def main():
    ...
    print("=" * 100)
    print("SHOT · SINGLE-KPI DML · E' ONLY · PASS-ALIGNED RF+RF PRIMARY")
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        if not eprime_def_columns:
            ...
            raise ValueError(
                ...
                "请先在L时刻freeze_frame上围绕shot end_location计算E' KPI。"
            )
    ...
    manifest = pd.DataFrame([{
        ...
        "L_estimand": "one defending Shot L KPI conditional on reduced background plus permitted Att(L)/Att(E') KPIs; target-matched Att controls are removed from both stages; Avg and centroid attacking controls are reciprocally excluded; every Def KPI and every actual-E KPI is excluded from X",
        "Eprime_estimand": "one defending Shot E' KPI conditional on reduced background plus permitted Att(L)/Att(E') KPIs; target-matched Att controls are removed from both stages; Avg and centroid attacking controls are reciprocally excluded; every Def KPI and every actual-E KPI is excluded from X",
        ...
        "interpretation_note": "Shot E' KPIs are measured around the corrected shot endpoint using the L-time freeze frame; every actual E-stage KPI is ignored.",
...
def joint_wald_test(
    ...
    fit = sm.OLS(y_res[mask], design).fit(
        cov_type="cluster",
        cov_kwds={
            "groups": groups[mask],
            "use_correction": True,
    ...
    try:
        ...
        test_type = "cluster-robust F/Wald"
    except Exception:
        try:
...
def build_rotating_target_selection(
    ...
    out["single_joint_direction_check"] = (
        ...
        "separate Shot single-KPI report before optimization"
    )
...
def joint_main():
    ...
    print("=" * 110)
    print("SHOT · PASS-STYLE ROTATING JOINT DML · L AND E' SEPARATELY")
    print(f"Input : {INPUT_PATH}")
    print(f"Output: {OUTPUT_XLSX}")
    print("=" * 110)
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        # Resolve the Shot home/away field to the canonical Pass-equivalent name.
    ...
    checklist = pd.DataFrame([
        ...
        {"requirement": "Shot event-type audit", "evidence": "Rows labelled dribble are excluded before modelling and counted", "sheet": "25_manifest", "status": "PASS"},
    ])
    ...
    manifest = pd.DataFrame([{
        ...
        "placebo_reps": PLACEBO_REPS,
        "second_stage": "joint OLS on OOF residuals + match-clustered SE",
        ...
        "forbidden_controls": "exact locations, endpoint coordinates, shot geometry, duration, freeze-frame count, and all actual-E KPIs",
        "Eprime_definition": "endpoint-centered KPI from shot-instant freeze frame; not arrival-time player positions",
...
# =============================================================================
# This is the active reviewer-facing Shot multivariable validation pipeline.
# The legacy rotating-joint functions above are retained only for traceability and
# are not called by __main__.

INTERACTION_OUTPUT_XLSX = (
    r"D:\pipeline\返修\DML部分\shot_dml_固定代表KPI_联合交互DML_审稿人最终报告.xlsx"
)
...
def fit_fixed_interaction_second_stage(
    ...
    add_fit = sm.OLS(y_res[mask], add_design).fit(
        cov_type="cluster",
        cov_kwds={
            "groups": groups[mask],
            "use_correction": True,
...
SINGLE_RESULT_XLSX = (
    r"D:\pipeline\返修\DML部分"
    r"\shot_dml_单指标_仅L_Avg与Centroid互斥_RF主模型_含训练fold内下采样敏感性.xlsx"
)
...
ALL_PASSED_INTERACTION_OUTPUT_XLSX = (
    r"D:\pipeline\返修\DML部分"
    r"\shot_dml_L_全部通过KPI_轮换联合交互DML_含训练fold内下采样敏感性.xlsx"
)
...
SINGLE_SCREEN_PLACEBO_P_MAX = 0.10
...
# Companion anchors are selected separately in every cluster-stage-family. A
# strictly passed KPI is always preferred. If no KPI in one family passes, the
# smallest-q available family KPI is retained only as a reference companion so
# the other strictly passed targets can still receive a three-mechanism model.
ANCHOR_SELECTION_RULE = (
    "strictly passed family KPI first; then smallest single-model global q; "
    "then largest RV_alpha05; then largest absolute 1-SD effect; "
    "a non-passed fallback is reference-companion-only"
)
...
def _choose_family_anchor(group: pd.DataFrame) -> pd.Series:
    ...
    ranked = ranked.sort_values(
        ["q_rank", "rv_rank", "effect_rank", "treatment"],
...
def build_screened_rotating_interaction_specs(
    ...
    The best strictly screened KPI in each family is the companion anchor. If one
    family has no strictly passed KPI, its smallest-q available KPI is used only as
    a reference companion; it is not added to the target list. A target replaces
    the anchor in its own family. Duplicate three-KPI sets are fitted once and
    mapped to all strictly passed target KPIs represented by the model.
    """
    available = set(available_kpis)
    all_candidates = screening_audit.loc[
        (screening_audit["cluster"] == cluster)
        & (screening_audit["stage"].astype(str) == str(stage))
    ].copy()
    all_candidates = all_candidates.loc[
        all_candidates["treatment"].isin(available)
        & all_candidates["joint_family"].notna()
    ].copy()
    candidates = all_candidates.loc[
        all_candidates["single_stage_pass"].map(_safe_bool)
    ].copy()

    required_families = ["local_advantage", "distance_centroid", "structure_shape"]
    anchors = OrderedDict()
    anchor_roles = OrderedDict()
    anchor_audit = []
    missing_families = []
    for family in required_families:
        family_rows = candidates.loc[candidates["joint_family"] == family].copy()
        anchor_role = "strictly_passed_companion"
        if family_rows.empty:
            family_rows = all_candidates.loc[
                all_candidates["joint_family"] == family
            ].copy()
            anchor_role = "reference_only_fallback_not_a_target"
        if family_rows.empty:
            missing_families.append(family)
            continue
        winner = _choose_family_anchor(family_rows)
        anchors[family] = winner["treatment"]
        anchor_roles[family] = anchor_role
        anchor_audit.append({
            "cluster": cluster,
            "stage": stage,
            "family": family,
            ...
            "anchor_single_stage_pass": _safe_bool(
                winner.get("single_stage_pass", False)
            ),
    ...
    for _, row in candidates.sort_values(["joint_family", "treatment"]).iterrows():
        ...
        target_map.append({
            ...
            "local_anchor_role": anchor_roles["local_advantage"],
            "distance_anchor_role": anchor_roles["distance_centroid"],
            "structure_anchor_role": anchor_roles["structure_shape"],
            "configured_family_anchor": anchors[family],
            "status": "RUN",
            "missing_kpis": "",
...
def fixed_interaction_main():
    ...
    print("=" * 110)
    print("SHOT · FIXED REPRESENTATIVE-KPI JOINT INTERACTION DML")
    print(f"Input : {INPUT_PATH}")
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        check_required_columns(df)
        if EXCLUDE_DRIBBLE_ROWS and TYPE_NAME_COL in df.columns:
            # Disabled for the Shot pipeline; retained only so the shared template
            # remains backwards compatible if the switch is changed manually.
            type_norm = df[TYPE_NAME_COL].astype("string").str.strip().str.casefold()
            dribble_mask = type_norm.isin(EXCLUDED_TYPE_NAMES)
            excluded_dribble_rows = int(dribble_mask.sum())
            df = df.loc[~dribble_mask].copy()
        df[OUTCOME_COL] = to_num(df[OUTCOME_COL])

        if not [c for c in df.columns if is_L_col(c) and not is_att_kpi(c)]:
            raise ValueError("No usable defending Shot KPI columns with suffix (L) were detected.")
    ...
    manifest = pd.DataFrame([{
        ...
        "rows_input_after_exclusion": len(df),
        "excluded_dribble_rows": excluded_dribble_rows,
        "matches_input": df[MATCH_COL].nunique(dropna=True),
        "outcome": OUTCOME_COL,
        ...
        "trim_grid": json.dumps(TRIM_GRID),
        "second_stage": "OLS on six OOF residualized basis terms + match-clustered SE",
...
def all_passed_interaction_main():
    ...
    print("=" * 110)
    print("SHOT · ALL SINGLE-STAGE-PASSED KPI ROTATING JOINT INTERACTION DML")
    print(f"Input : {INPUT_PATH}")
    print(f"Output: {ALL_PASSED_INTERACTION_OUTPUT_XLSX}")
    print("=" * 110)

    excluded_non_shot_rows = 0
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        df = read_input_table(INPUT_PATH, sheet_name=SHEET_NAME)
        df.columns = [str(column).strip() for column in df.columns]
        if HOME_AWAY_COL not in df.columns:
        ...
        if EXCLUDE_DRIBBLE_ROWS and TYPE_NAME_COL in df.columns:
            type_norm = df[TYPE_NAME_COL].astype("string").str.strip().str.casefold()
            dribble_mask = type_norm.isin(EXCLUDED_TYPE_NAMES)
            excluded_non_shot_rows = int(dribble_mask.sum())
            df = df.loc[~dribble_mask].copy()
        df[OUTCOME_COL] = to_num(df[OUTCOME_COL])

        if not [c for c in df.columns if is_L_col(c) and not is_att_kpi(c)]:
            raise ValueError(
                "No usable defending Shot KPI columns with suffix (L) were detected."
            )
        ...
        for stage in ACTIVE_STAGES:
            ...
            stage_specs, stage_target_map, stage_anchor_rows = (
                build_screened_rotating_interaction_specs(
                    cluster, stage, available, single_screening_audit
                )
            )
            selection_rows.extend(stage_target_map)
    ...
    undersample_sampling_audit = concat(undersample_sampling_frames)
    if (
        not undersample_target_compare.empty
        and not undersample_sampling_audit.empty
        and "sampling_applied" in undersample_sampling_audit.columns
    ):
        trigger_rows = []
        for keys, group in undersample_sampling_audit.groupby(
            ["cluster", "stage", "joint_spec"], dropna=False
        ):
            applied = group["sampling_applied"].map(_safe_bool)
            trigger_rows.append({
                "cluster": keys[0],
                "stage": keys[1],
                "joint_spec": keys[2],
                "undersampling_applied_any": bool(applied.any()),
                "n_training_sets_audited": int(len(group)),
                "n_training_sets_undersampled": int(applied.sum()),
            })
        trigger_summary = pd.DataFrame(trigger_rows)
        undersample_target_compare = undersample_target_compare.merge(
            trigger_summary,
            on=["cluster", "stage", "joint_spec"],
            how="left",
        )
        not_triggered = ~undersample_target_compare[
            "undersampling_applied_any"
        ].fillna(False).astype(bool)
        undersample_target_compare.loc[
            not_triggered, "imbalance_sensitivity_class"
        ] = "NOT_TRIGGERED_NO_CLASS_IMBALANCE"
        undersample_target_compare.loc[
            not_triggered, "imbalance_sensitivity_flag"
        ] = False
        undersample_target_compare.loc[
            not_triggered, "optimizer_eligible_after_undersampling"
        ] = False
    ...
    if not target_summary.empty and not undersample_target_compare.empty:
        ...
        compare_cols = compare_keys + [c for c in [
            ...
            "undersampling_applied_any",
            "n_training_sets_audited",
            "n_training_sets_undersampled",
    ...
    checklist = pd.DataFrame([
        {"requirement": "Multivariable causal estimation", "evidence": "Every strictly screened Shot-L target is rotated through a three-mechanism interaction model; any non-passed family fallback is reference-companion-only and audited", "sheet": "02_target_AME_report / 03_all_model_AME / 04_term_report / 32_anchor_plan", "status": "REPORTED" if not main_summary.empty else "MISSING"},
        ...
        {"requirement": "Class-imbalance sensitivity", "evidence": "Training-fold-only 1:2 undersampling is attempted and the audit explicitly distinguishes applied from not-triggered folds; validation/test distributions, D|X and second stage remain unchanged", "sheet": "36-45 undersampling sheets", "status": "REPORTED" if not undersample_target_compare.empty else "NOT_RUN"},
    ...
    manifest = pd.DataFrame([{
        ...
        "rows_input_after_exclusion": len(df),
        "excluded_non_shot_rows": excluded_non_shot_rows,
        "matches_input": df[MATCH_COL].nunique(dropna=True),
        "outcome": OUTCOME_COL,
        ...
        "OVB": "partial R2 and robustness values reported; detailed observed-covariate benchmarking remains in the single-KPI workbook",
        "stage_policy": "Shot uses L only; every (E) and E' KPI is ignored as treatment and covariate",
        "external_validation": "not run because no external dataset is available",
        "xgboost_available": HAS_XGBOOST,
    }])
    ...
    tables = OrderedDict([
        ...
        ("44_undersample_sampling", undersample_sampling_audit),

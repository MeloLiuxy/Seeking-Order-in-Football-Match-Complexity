...
CLUSTER_COL_CANDIDATES = ["cluster_id", "cluster_id_loc", "cluster_id_loc_only", "cluster"]

BASE_SPLITS    = 5
RANDOM_SEED    = 42

# 默认 nuisance 训练折下采样（只影响 nuisance，不改变二阶段样本）
...
# 原始字符列如果也在表里，不参与 DML（避免 to_numeric 后全变 NaN）
...
def looks_like_meta(name: str):
    ...
    # 白名单：这几个虽然含 minute，但它们是需要强制保留的控制变量
        ...
        if mask.sum() >= 20:
            ...
            for b in range(GPS_BINS):
                ...
                if m2.sum() < 5:
                    ...
                    lo = float(np.nanmin(d[m2]))
                    hi = float(np.nanmax(d[m2]))
    ...
    with pd.ExcelWriter(OUTPUT_XLSX, engine="xlsxwriter") as w:
        ...
        for cid in clusters:
            ...
            robL_t0, _, _ = run_block_oneD(
                sub=sub,
                block_label=f"L_cluster_{cid}_trim0",
                D_cols=L_nonatt,
                X_builder=X_builder_L,
                seed=RANDOM_SEED,
                trim_top_frac=0.0,
            ...
            robL_nu, _, _ = run_block_oneD(
                sub=sub,
                block_label=f"L_cluster_{cid}_noUS",
                D_cols=L_nonatt,
                X_builder=X_builder_L,
                seed=RANDOM_SEED,
                trim_top_frac=TRIM_TOP_FRAC_DEFAULT,
            ...
            robL_ols, _, _ = run_block_oneD(
                sub=sub,
                block_label=f"L_cluster_{cid}_OLS",
                D_cols=L_nonatt,
                X_builder=X_builder_L,
                seed=RANDOM_SEED,
                trim_top_frac=TRIM_TOP_FRAC_DEFAULT,

...
# 原始特征数据（shot）
SHOT_FEATURES_PATH = r""
SHOT_FEATURES_SHEET = 0
...
MIN_FAIL_SAMPLES = 1   # 你原代码就是 1，这里保持
...
L_cluster_0\tAdv_10(L)\t-0.049370926\t-0.049946443\t-5.887431493\t0.013322965\t2622\t0.559496568\t0.422395982\t1.7988E-08\t2.0262E-05
L_cluster_0\tAdv_5(L)\t-0.062442647\t-0.064419219\t-5.864790529\t0.013222043\t2622\t0.559496568\t0.318047876\t1.7988E-08\t8.68447E-08
L_cluster_0\tAvg_1_Def(L)\t-0.070951792\t-0.018166616\t-5.584942478\t0.012005113\t2622\t0.559496568\t0.207879874\t6.23407E-08\t0.162900954
L_cluster_0\tAvg_3_Def(L)\t-0.079636196\t-0.072101598\t-5.302300875\t0.010833586\t2622\t0.559496568\t0.169322198\t2.28704E-07\t4.19988E-08
L_cluster_0\tAvg_5_Def(L)\t-0.085980029\t-0.077825988\t-4.319660676\t0.007216522\t2622\t0.559496568\t0.114177722\t2.50031E-05\t4.19988E-08
L_cluster_1\tAvg_3_Def(L)\t-0.053324776\t-0.057772376\t-4.316591923\t0.004710489\t4020\t0.415671642\t0.448103104\t6.33827E-05\t2.29257E-05
L_cluster_1\tAvg_5_Def(L)\t-0.061361532\t-0.061986055\t-4.42024137\t0.00493829\t4020\t0.415671642\t0.55350115\t6.33827E-05\t3.27886E-05
L_cluster_2\tAdv_10(L)\t-0.056103176\t-0.057411854\t-4.477225679\t0.009297368\t2182\t0.49908341\t0.127434252\t1.51239E-05\t1.20114E-05
L_cluster_2\tAdv_5(L)\t-0.058539257\t-0.05715818\t-4.651943578\t0.010029742\t2182\t0.49908341\t0.138845584\t8.76856E-06\t1.20114E-05
L_cluster_2\tAvg_3_Def(L)\t-0.072553229\t-0.0735741\t-5.233883871\t0.012662301\t2182\t0.49908341\t0.237665841\t1.32788E-06\t9.76027E-07
L_cluster_2\tAvg_5_Def(L)\t-0.073410271\t-0.075654636\t-4.936787224\t0.01128133\t2182\t0.49908341\t0.350362902\t3.1768E-06\t1.56256E-06
"""
...
def load_shot_features(path, sheet=0):
...
def load_robust_results(path=None, inline_text=None):
    ...
    pattern = re.compile(r"^([LE])_cluster_(\d+)$", re.IGNORECASE)
...
def process_one_cluster(cid, sub_all, robust_df_full):
    ...
    robust_df = robust_df_full[robust_df_full["side"] == "L"].copy()
    if robust_df.empty:
...
def run_for_L_stage_parallel(df_all, cluster_to_results, writer):
    print(f"\n========== 并行计算阶段 L ==========")
    ...
    for r in ok_results:
        ...
        sheet_name = f"L_cluster_{cid}_policy"[:31]
    ...
    if summary_rows:
        ...
        df_summary.to_excel(writer, index=False, sheet_name="summary_L")
...
def main():
    ...
    df_all = load_shot_features(SHOT_FEATURES_PATH, SHOT_FEATURES_SHEET)
    ...
    with pd.ExcelWriter(OUTPUT_POLICY_XLSX, engine="xlsxwriter") as writer:
        run_for_L_stage_parallel(df_all, cluster_to_results, writer)

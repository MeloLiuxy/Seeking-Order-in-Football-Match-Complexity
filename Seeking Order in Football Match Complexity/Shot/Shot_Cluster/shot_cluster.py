# -*- coding: utf-8 -*-
"""
Carry · All single-stage-passed KPI rotating joint interaction DML · L and E' separately
===========================================================================
...
RANDOM_SEED = 42
N_SPLITS = 5
ACTIVE_STAGES = ("E'",)  # Current paper scope; add "L" here to run both stages
...
# Exact reference copied from the final Carry single-KPI main model.
...
# Therefore the nuisance models do not condition on exact carry location/geometry,
...
# The former Carry Model A is retained as the prespecified anchor combination.
# A target replaces the anchor in its own family; the other two anchors stay fixed.
...
def validate_primary_rf_alignment() -> None:
    ...
    if mismatches:
        raise ValueError(
            "Primary RF settings no longer match the final Carry single-KPI model: "
...
def main():
    ...
    print("=" * 100)
    print("CARRY · SINGLE-KPI DML · E' ONLY · PASS-ALIGNED RF+RF PRIMARY")
...
def build_rotating_target_selection(
    ...
    out["single_joint_direction_check"] = (
        ...
        "separate Carry single-KPI report before optimization"
    )
...
def joint_main():
    ...
    print("=" * 110)
    print("CARRY · PASS-STYLE ROTATING JOINT DML · L AND E' SEPARATELY")
    print(f"Input : {INPUT_PATH}")
    print(f"Output: {OUTPUT_XLSX}")
    print("=" * 110)
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        # Resolve the Carry home/away field to the canonical Pass-equivalent name.
    ...
    checklist = pd.DataFrame([
        ...
        {"requirement": "Carry event-type audit", "evidence": "Rows labelled dribble are excluded before modelling and counted", "sheet": "25_manifest", "status": "PASS"},
    ])
    ...
    manifest = pd.DataFrame([{
        ...
        "forbidden_controls": "exact locations, endpoint coordinates, carry geometry, duration, freeze-frame count, and all actual-E KPIs",
        "Eprime_definition": "endpoint-centered KPI from carry-instant freeze frame; not arrival-time player positions",
...
# =============================================================================
# This is the active reviewer-facing Carry multivariable validation pipeline.
...
INTERACTION_OUTPUT_XLSX = (
    r"D:\pipeline\返修\DML部分\carry_dml_固定代表KPI_联合交互DML_审稿人最终报告.xlsx"
)
...
SINGLE_RESULT_XLSX = (
    r"D:\pipeline\返修\DML部分\carry_dml_单指标_L和Eprime_RF主模型_含训练fold内下采样敏感性.xlsx"
)
...
ALL_PASSED_INTERACTION_OUTPUT_XLSX = (
    r"D:\pipeline\返修\DML部分\carry_dml_Eprime_全部通过KPI_轮换联合交互DML_含训练fold内下采样敏感性.xlsx"
)
...
ANCHOR_SELECTION_RULE = (
    "smallest single-model global q; then largest RV_alpha05; "
    "then largest absolute 1-SD effect"
)
...
def fixed_interaction_main():
    ...
    print("=" * 110)
    print("CARRY · FIXED REPRESENTATIVE-KPI JOINT INTERACTION DML")
    print(f"Input : {INPUT_PATH}")
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        if not [c for c in df.columns if is_Eprime_col(c) and not is_att_kpi(c)]:
            raise ValueError("No usable Def(E') KPI columns were detected.")
...
def all_passed_interaction_main():
    ...
    print("=" * 110)
    print("CARRY · ALL SINGLE-STAGE-PASSED KPI ROTATING JOINT INTERACTION DML")
    print(f"Input : {INPUT_PATH}")
    ...
    manifest = pd.DataFrame([{
        ...
        "OVB": "partial R2 and robustness values reported; detailed observed-covariate benchmarking remains in the single-KPI workbook",
        "actual_E_policy": "actual (E) KPIs ignored; current run analyzes E' only",
        "external_validation": "not run because no external dataset is available",
        "xgboost_available": HAS_XGBOOST,
    }])

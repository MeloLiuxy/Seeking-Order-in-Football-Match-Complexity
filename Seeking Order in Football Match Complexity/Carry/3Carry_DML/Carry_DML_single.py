# -*- coding: utf-8 -*-
"""
Carry · Single-KPI DML · L and E' · Pass-identical covariate design
=================================================================

Main design
-----------
The Carry L and E' stages are analysed separately.

Y:
    success_def

D:
    one defending-side L or E' KPI at a time.

X:
    the same reduced background variables and attacking-side KPI controls used
    in the final Pass single-KPI DML:
      - reduced non-spatial background;
      - permitted Att(L) KPIs;
      - permitted Att(E') KPIs;
      - categorical team/player-position/play-pattern/score-state/home-away
        indicators.

For each target D, the complete matching attacking KPI series is removed from
both L and E'. No defending KPI from L or E' enters X.

Actual E-stage KPIs
-------------------
Every column ending in "(E)" is ignored completely:
    - never a treatment;
    - never a covariate;
    - never used in a joint or nonlinear model.

This script does not rename actual E KPIs to E'. The input file must already
contain valid E' KPI columns calculated around the Carry end location using the
L-time freeze frame.

Main nuisance learners:
    Y|X: RandomForestClassifier + nested match-grouped Platt calibration
...
Other settings:
    ...
    match-clustered second-stage standard errors;
    overlap/support diagnostics;
    within-match placebo;
    fold and leave-one-team-out stability;
    global and within-cluster BH-FDR;
    reviewer-facing omitted-variable sensitivity analysis:
      * robustness values for the point estimate and alpha=0.05 inference;
      * observed-covariate group benchmarking by partial R2;
      * 0.5x, 1x, and 2x benchmark-strength bias-adjusted effects;
      * equal-strength and two-dimensional sensitivity grids.

The script is the single-KPI main model. Joint DML and nonlinear treatment
checks remain disabled. External validation is deliberately not included here.
...
using majority-class random undersampling inside every inner/outer TRAINING fold.
The inner calibration validation rows and outer test rows retain the original
class distribution. The treatment nuisance model and second-stage sample are not
undersampled. The default target minority:majority ratio is 1:2.
"""
...
HOME_AWAY_COL = "carry_team_home_away"
...
# Class-imbalance sensitivity. The primary model still uses the original class
# distribution. Set to ("E'",) if only the manuscript-reported E' stage is needed.
...
# Carry-only data-quality guard.
EXCLUDE_DRIBBLE_ROWS = True
TYPE_NAME_COL = "type_name"
EXCLUDED_TYPE_NAMES = {"dribble"}
...
INCLUDE_ATT_EPRIME_IN_L_X = True
...
# Diagnostics
PLACEBO_REPS = 200          # reuses OOF residuals; does not refit 200 times
...
# Reviewer-facing omitted-variable sensitivity analysis.
# This module is attached to the single-KPI RF DML second-stage estimates.
# It does not claim to remove unobserved confounding; it quantifies how strong
# omitted confounding would need to be to attenuate the reported effect.
RUN_OVB_SENSITIVITY = True
OVB_ALPHA = 0.05
OVB_BENCHMARK_MULTIPLIERS = (0.5, 1.0, 2.0)
OVB_EQUAL_STRENGTH_R2_GRID = (0.005, 0.01, 0.02, 0.05, 0.10, 0.15, 0.20)
OVB_CONTOUR_R2_GRID = tuple(np.round(np.linspace(0.0, 0.25, 26), 4))
OVB_MAX_PARTIAL_R2 = 0.95
OVB_MIN_BENCHMARK_COLUMNS = 1
OVB_CONTOUR_ONLY_FOR_DISCUSSION_CANDIDATES = True
...
# All score variables use the state BEFORE the current carry.
...
# but endpoint coordinates, carry geometry, duration, and FF player count are NOT in X.
...
RAW_OR_POST_EVENT_PATTERNS = [
    r"^id$", r"^type$", r"^pass$", r"^carry$", r"freeze_frame", r"back_ff",
...
def undersample_binary_training_indices(
    ...
    """Return training-row indices after deterministic random majority undersampling.

    The target ratio follows imbalanced-learn semantics:
        n_minority / n_majority = target_minority_to_majority_ratio.
    All minority rows are retained. No validation or test rows are sampled.
    """
    ...
    desired_majority = int(np.floor(n_minority / ratio))
    desired_majority = max(n_minority, desired_majority)
    ...
    achieved = sampled_minority / sampled_majority if sampled_majority else np.nan
    ...
    audit.update({
        ...
        "achieved_minority_to_majority_ratio": float(achieved),
    })
...
def fit_predict_rf_outcome_undersampled(
    ...
    """Fit the RF outcome learner after training-only majority undersampling."""
    imputer = SimpleImputer(strategy="median")
    ...
    model.fit(Xtr_full[sample_idx], np.asarray(y_train, dtype=int)[sample_idx])
...
def nested_group_platt_predict_undersampled(
    ...
    """Nested grouped Platt calibration for the undersampled RF sensitivity.

    Only each inner/outer model-training subset is undersampled. Inner validation
    rows used to fit Platt scaling and the outer test rows remain untouched.
    """
...
def crossfit_undersampled_rf_y(
    ...
    """Grouped OOF outcome predictions for the undersampling sensitivity branch."""
    n = len(y)
    ...
    for fold, (tr, te) in enumerate(splits):
        ...
        calibrated_outer = safe_binary_metrics(y[te], p)
        for key, value in raw_outer.items():
            audit[f"outer_raw_{key}"] = value
        for key, value in calibrated_outer.items():
    ...
    return {
        ...
        "sampling_audit": (
            pd.concat(sampling_rows, ignore_index=True) if sampling_rows else pd.DataFrame()
        ),
    }
...
# -----------------------------------------------------------------------------
# Reviewer-facing omitted-variable sensitivity and observed-covariate benchmarks
# -----------------------------------------------------------------------------

def ovb_adjusted_statistics(
    estimate: float,
    se: float,
    dof: int,
    r2_dz_x: float,
    ...
    """Bias-adjusted coefficient, SE, t, p and CI for hypothetical confounding.

    The formulas follow the partial-R2 omitted-variable-bias parameterization.
    Because the main manuscript inference uses match-clustered standard errors,
    this implementation applies the formulas to the cluster-robust coefficient,
    cluster-robust SE, and match-level degrees of freedom. It is therefore
    reported explicitly as a cluster-df sensitivity approximation rather than as
    an exact replacement for a fully cluster-specific sensitivity estimator.
    """
    values = [estimate, se, r2_dz_x, r2_yz_dx]
    if not all(np.isfinite(v) for v in values) or se <= 0 or dof <= 1:
        return {
            "ovb_bias": np.nan,
            "adjusted_estimate": np.nan,
            "adjusted_se": np.nan,
            "adjusted_t": np.nan,
            "adjusted_p": np.nan,
            "adjusted_ci_low": np.nan,
            "adjusted_ci_high": np.nan,
            "adjusted_sign_preserved": np.nan,
            "adjusted_significant_alpha": np.nan,
        }

    r2d = float(np.clip(r2_dz_x, 0.0, OVB_MAX_PARTIAL_R2))
    r2y = float(np.clip(r2_yz_dx, 0.0, OVB_MAX_PARTIAL_R2))
    denom = max(1e-12, 1.0 - r2d)

    bias = float(se * np.sqrt(dof * r2y * r2d / denom))
    direction = np.sign(estimate) if reduce else -np.sign(estimate)
    adjusted_estimate = float(estimate - direction * bias)
    adjusted_se = float(
        se * np.sqrt(((1.0 - r2y) / denom) * (dof / max(1.0, dof - 1.0)))
    )
    dof_adj = max(1, int(dof - 1))
    adjusted_t = adjusted_estimate / adjusted_se if adjusted_se > 0 else np.nan
    adjusted_p = (
        float(2.0 * stats.t.sf(abs(adjusted_t), dof_adj))
        if np.isfinite(adjusted_t) else np.nan
    )
    tcrit = float(stats.t.ppf(1.0 - alpha / 2.0, dof_adj))
    ci_low = float(adjusted_estimate - tcrit * adjusted_se)
    ci_high = float(adjusted_estimate + tcrit * adjusted_se)
    sign_preserved = bool(
        np.sign(adjusted_estimate) == np.sign(estimate)
        and abs(adjusted_estimate) > 1e-15
    )
    ...
    return {
        ...
        "adjusted_t": float(adjusted_t) if np.isfinite(adjusted_t) else np.nan,
        "adjusted_p": adjusted_p,
        "adjusted_ci_low": ci_low,
        "adjusted_ci_high": ci_high,
        "adjusted_sign_preserved": sign_preserved,
        "adjusted_significant_alpha": significant,
    }


def _standardize_benchmark_matrix(X_df: pd.DataFrame) -> Tuple[np.ndarray, List[str]]:
    """Median-impute and standardize controls for stable linear-projection benchmarks."""
    if X_df is None or X_df.empty:
        return np.empty((len(X_df), 0)), []
    imputer = SimpleImputer(strategy="median")
    X = imputer.fit_transform(X_df.values.astype(float))
    sd = np.std(X, axis=0, ddof=1)
    keep = np.isfinite(sd) & (sd > 1e-12)
    if not np.any(keep):
        return np.empty((len(X_df), 0)), []
    X = X[:, keep]
    cols = list(np.asarray(X_df.columns, dtype=object)[keep])
    mean = np.mean(X, axis=0)
    sd = np.std(X, axis=0, ddof=1)
    X = (X - mean) / sd
    return X.astype(float), cols


def _crossprod_sse(design: np.ndarray, outcome: np.ndarray, keep_idx: Sequence[int]) -> float:
    """OLS SSE from preselected columns, using stable pseudo-inverse cross-products."""
    idx = np.asarray(list(keep_idx), dtype=int)
    if idx.size == 0:
        return np.nan
    Z = design[:, idx]
    xtx = Z.T @ Z
    ...
    sse = float(outcome @ outcome - xty @ beta)
    return max(0.0, sse)


def _partial_r2_from_nested_sse(sse_restricted: float, sse_full: float) -> float:
    if not np.isfinite(sse_restricted) or not np.isfinite(sse_full) or sse_restricted <= 1e-12:
        return np.nan
    value = (sse_restricted - sse_full) / sse_restricted
    return float(np.clip(value, 0.0, 1.0))


def build_ovb_benchmark_groups(
    X_columns: Sequence[str],
    block: Dict[str, object],
) -> OrderedDict:
    """Create prespecified observed-control groups for confounding benchmarks."""
    available = set(map(str, X_columns))

    def existing(cols: Sequence[str]) -> List[str]:
        return [str(c) for c in cols if str(c) in available]

    groups = OrderedDict()
    groups["match_context_numeric"] = existing(block.get("base_controls", []))
    groups["team"] = existing(block.get("team_control_cols", []))
    groups["player_position"] = existing(block.get("position_control_cols", []))
    groups["play_pattern"] = existing(block.get("play_pattern_control_cols", []))
    groups["score_state"] = existing(block.get("score_state_control_cols", []))
    groups["home_away"] = existing(block.get("home_away_control_cols", []))

    attacking_l = [c for c in X_columns if is_L_col(c) and is_att_kpi(c)]
    attacking_ep = [c for c in X_columns if is_Eprime_col(c) and is_att_kpi(c)]
    groups["attacking_KPIs_L"] = existing(attacking_l)
    groups["attacking_KPIs_Eprime"] = existing(attacking_ep)
    groups["all_attacking_KPIs"] = existing(attacking_l + attacking_ep)

    categorical = (
        list(block.get("team_control_cols", []))
        + list(block.get("position_control_cols", []))
        + list(block.get("play_pattern_control_cols", []))
        + list(block.get("score_state_control_cols", []))
        + list(block.get("home_away_control_cols", []))
    )
    groups["all_categorical_context"] = existing(categorical)
    groups["all_observed_context"] = existing(
        list(block.get("base_controls", [])) + categorical
    )

    # Add individual numeric context controls so the workbook also contains
    # transparent single-variable benchmarks, not only grouped benchmarks.
    for c in block.get("base_controls", []):
        cols = existing([c])
        if cols:
            groups[f"single::{c}"] = cols

    # Remove empty and exact duplicate groups while preserving order.
    cleaned = OrderedDict()
    seen = set()
    for name, cols in groups.items():
        cols = uniq_keep_order(cols)
        key = tuple(cols)
        if len(cols) < OVB_MIN_BENCHMARK_COLUMNS or key in seen:
            continue
        seen.add(key)
        cleaned[name] = cols
    return cleaned
...
def observed_covariate_benchmark_partial_r2(
    X_df: pd.DataFrame,
    D: np.ndarray,
    y: np.ndarray,
    block: Dict[str, object],
    ...
    """Partial-R2 strength of observed covariate groups with D and Y.

    For treatment strength, the group partial R2 is from D ~ X.
    For outcome strength, it is from Y ~ D + X. These are linear-projection
    benchmarks used to calibrate plausible omitted-confounder scenarios. They do
    not replace the nonlinear cross-fitted nuisance models used by the main DML.
    """
    X, cols = _standardize_benchmark_matrix(X_df)
    if X.shape[1] == 0:
        return pd.DataFrame()
    ...
    if not np.isfinite(d_sd) or d_sd <= 1e-12:
        return pd.DataFrame()
    dz = (d - float(np.mean(d))) / d_sd

    n = len(yv)
    intercept = np.ones((n, 1), dtype=float)
    design_d = np.column_stack([intercept, X])
    design_y = np.column_stack([intercept, dz, X])
    all_d_idx = list(range(design_d.shape[1]))
    all_y_idx = list(range(design_y.shape[1]))
    sse_d_full = _crossprod_sse(design_d, dz, all_d_idx)
    sse_y_full = _crossprod_sse(design_y, yv, all_y_idx)

    col_to_pos = {c: i for i, c in enumerate(cols)}
    groups = build_ovb_benchmark_groups(cols, block)
    rows = []
    for group_name, group_cols in groups.items():
        x_positions = [col_to_pos[c] for c in group_cols if c in col_to_pos]
        if not x_positions:
            continue
        # X begins at index 1 in design_d and index 2 in design_y.
        remove_d = {1 + p for p in x_positions}
        remove_y = {2 + p for p in x_positions}
        keep_d = [i for i in all_d_idx if i not in remove_d]
        keep_y = [i for i in all_y_idx if i not in remove_y]
        sse_d_restricted = _crossprod_sse(design_d, dz, keep_d)
        sse_y_restricted = _crossprod_sse(design_y, yv, keep_y)
        r2d = _partial_r2_from_nested_sse(sse_d_restricted, sse_d_full)
        r2y = _partial_r2_from_nested_sse(sse_y_restricted, sse_y_full)
        if not np.isfinite(r2d) or not np.isfinite(r2y):
            continue
        bias_factor = float(np.sqrt(r2y * r2d / max(1e-12, 1.0 - r2d)))
        rows.append({
            "cluster": cluster,
            "stage": stage,
            "treatment": treatment,
            "benchmark_group": group_name,
            "n_benchmark_columns": len(group_cols),
            "benchmark_columns": " | ".join(group_cols),
            "benchmark_partial_r2_D_given_other_X": r2d,
            "benchmark_partial_r2_Y_given_D_other_X": r2y,
            "benchmark_bias_factor": bias_factor,
            "benchmark_geometric_mean_r2": float(np.sqrt(r2d * r2y)),
            "benchmark_method": "linear-projection group partial R2; main DML nuisances remain nonlinear and cross-fitted",
        })
    return pd.DataFrame(rows)


def build_ovb_scenario_tables(
    effect_row: pd.Series,
    benchmark_df: pd.DataFrame,
    cluster,
    stage: str,
    treatment: str,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Create equal-strength and observed-benchmark-scaled OVB scenarios."""
    estimate = float(effect_row["theta_per_1sd"])
    se = float(effect_row["se_per_1sd"])
    dof = int(effect_row["cluster_se_df"])

    equal_rows = []
    for r2 in OVB_EQUAL_STRENGTH_R2_GRID:
        stats_adj = ovb_adjusted_statistics(
            estimate, se, dof, r2, r2, alpha=OVB_ALPHA, reduce=True
        )
        equal_rows.append({
            "cluster": cluster,
            "stage": stage,
            "treatment": treatment,
            "scenario_type": "equal_strength_manual_grid",
            "r2_DZ_given_X": float(r2),
            "r2_YZ_given_DX": float(r2),
            "benchmark_group": "manual_equal_strength",
            "benchmark_multiplier": np.nan,
            "original_theta_per_1sd": estimate,
            "original_se_per_1sd": se,
            "original_dof": dof,
            **stats_adj,
        })
    ...
    if benchmark_df is not None and not benchmark_df.empty:
        for _, b in benchmark_df.iterrows():
            base_d = float(b["benchmark_partial_r2_D_given_other_X"])
            base_y = float(b["benchmark_partial_r2_Y_given_D_other_X"])
            for multiplier in OVB_BENCHMARK_MULTIPLIERS:
                r2d = float(np.clip(multiplier * base_d, 0.0, OVB_MAX_PARTIAL_R2))
                r2y = float(np.clip(multiplier * base_y, 0.0, OVB_MAX_PARTIAL_R2))
                stats_adj = ovb_adjusted_statistics(
                    estimate, se, dof, r2d, r2y, alpha=OVB_ALPHA, reduce=True
                )
                benchmark_rows.append({
                    "cluster": cluster,
                    "stage": stage,
                    "treatment": treatment,
                    "scenario_type": "observed_benchmark_scaled",
                    "benchmark_group": b["benchmark_group"],
                    "benchmark_multiplier": float(multiplier),
                    "benchmark_columns": b["benchmark_columns"],
                    "benchmark_partial_r2_D": base_d,
                    "benchmark_partial_r2_Y": base_y,
                    "r2_DZ_given_X": r2d,
                    "r2_YZ_given_DX": r2y,
                    "original_theta_per_1sd": estimate,
                    "original_se_per_1sd": se,
    ...
    return pd.DataFrame(equal_rows), pd.DataFrame(benchmark_rows)


def build_ovb_summary(
    main_df: pd.DataFrame,
    benchmark_df: pd.DataFrame,
    benchmark_scenarios_df: pd.DataFrame,
) -> pd.DataFrame:
    """One reviewer-friendly OVB row per main single-KPI DML estimate."""
    if main_df is None or main_df.empty:
        return pd.DataFrame()
    keys = ["cluster", "stage", "treatment"]
    rows = []
    for key, effect in main_df.groupby(keys, dropna=False):
        effect_row = effect.iloc[0]
        rec = {k: v for k, v in zip(keys, key)}
        rec.update({
            "theta_per_1sd": effect_row.get("theta_per_1sd", np.nan),
            "se_per_1sd": effect_row.get("se_per_1sd", np.nan),
            "p_value": effect_row.get("p_value", np.nan),
            "q_global": effect_row.get("q_global", np.nan),
            "partial_r2_treatment_outcome": effect_row.get("partial_r2_cluster_t", np.nan),
            "robustness_value_point": effect_row.get("robustness_value_point", np.nan),
            "robustness_value_alpha05": effect_row.get("robustness_value_alpha05", np.nan),
            "sensitivity_inference_note": "partial-R2 OVB formulas applied to cluster-robust estimate/SE and match-level df",
        })

        if benchmark_df is not None and not benchmark_df.empty:
            mask = np.ones(len(benchmark_df), dtype=bool)
            for k, v in rec.items():
                if k in keys:
                    mask &= benchmark_df[k].astype(str).values == str(v)
            bb = benchmark_df.loc[mask].copy()
            if not bb.empty:
                strongest = bb.sort_values("benchmark_bias_factor", ascending=False).iloc[0]
                rec.update({
                    "strongest_observed_benchmark": strongest["benchmark_group"],
                    "strongest_benchmark_columns": strongest["benchmark_columns"],
                    "strongest_benchmark_r2_D": strongest["benchmark_partial_r2_D_given_other_X"],
                    "strongest_benchmark_r2_Y": strongest["benchmark_partial_r2_Y_given_D_other_X"],
                    "strongest_benchmark_bias_factor": strongest["benchmark_bias_factor"],
                })

                if benchmark_scenarios_df is not None and not benchmark_scenarios_df.empty:
                    ss = benchmark_scenarios_df.copy()
                    mm = np.ones(len(ss), dtype=bool)
                    for k, v in zip(keys, key):
                        mm &= ss[k].astype(str).values == str(v)
                    mm &= ss["benchmark_group"].astype(str).values == str(strongest["benchmark_group"])
                    ss = ss.loc[mm]
                    for mult, label in [(1.0, "1x"), (2.0, "2x")]:
                        z = ss.loc[np.isclose(ss["benchmark_multiplier"], mult)]
                        if not z.empty:
                            z = z.iloc[0]
                            rec[f"adjusted_theta_{label}_strongest_benchmark"] = z["adjusted_estimate"]
                            rec[f"adjusted_ci_low_{label}_strongest_benchmark"] = z["adjusted_ci_low"]
                            rec[f"adjusted_ci_high_{label}_strongest_benchmark"] = z["adjusted_ci_high"]
                            rec[f"adjusted_p_{label}_strongest_benchmark"] = z["adjusted_p"]
                            rec[f"sign_preserved_{label}_strongest_benchmark"] = z["adjusted_sign_preserved"]
                            rec[f"significant_{label}_strongest_benchmark"] = z["adjusted_significant_alpha"]
        rows.append(rec)
    return pd.DataFrame(rows)


def build_ovb_contour_grid(main_df: pd.DataFrame) -> pd.DataFrame:
    """Two-dimensional sensitivity grid for plotting or appendix inspection."""
    if main_df is None or main_df.empty:
        return pd.DataFrame()
    source = main_df.copy()
    if OVB_CONTOUR_ONLY_FOR_DISCUSSION_CANDIDATES and "candidate_for_discussion" in source.columns:
        source = source.loc[source["candidate_for_discussion"].fillna(False)].copy()
    rows = []
    for _, effect in source.iterrows():
        estimate = float(effect["theta_per_1sd"])
        se = float(effect["se_per_1sd"])
        dof = int(effect["cluster_se_df"])
        for r2d in OVB_CONTOUR_R2_GRID:
            for r2y in OVB_CONTOUR_R2_GRID:
                adj = ovb_adjusted_statistics(
                    estimate, se, dof, float(r2d), float(r2y),
                    alpha=OVB_ALPHA, reduce=True,
                )
                rows.append({
                    "cluster": effect["cluster"],
                    "stage": effect["stage"],
                    "treatment": effect["treatment"],
                    "r2_DZ_given_X": float(r2d),
                    "r2_YZ_given_DX": float(r2y),
                    "original_theta_per_1sd": estimate,
                    **adj,
                })
    return pd.DataFrame(rows)
...
def fit_clustered_second_stage(
    ...
    for j, name in enumerate(names):
        ...
        rows.append({
            ...
            "n_matches": n_groups,
            "cluster_se_df": dof_cluster,
            "second_stage": "OLS + match-clustered SE",
...
def build_inventory(sub: pd.DataFrame) -> Dict[str, object]:
    ...
    D_L = [c for c in all_L if c not in set(L_att)]
    D_E = [c for c in all_E if c not in set(E_att)]
...
def build_target_dataset(
    ...
    """Build the one-KPI DML dataset with matched Att-series exclusion.
    ...
    if len(selected) != 1:
        raise ValueError(
            "This script is the single-KPI main model; exactly one treatment "
            "must be supplied to build_target_dataset()."
        )
    ...
    if stage == "L":
        ...
        estimand = (
            "conditional effect of one defending L KPI given reduced pre-event "
        ...
        estimand = (
            "conditional effect of one defending E' KPI measured around the "
            "carry endpoint in the L-time freeze frame, given reduced pre-event "
            "background and permitted Att(L)/Att(E') controls; the target-matched "
            "attacking series is removed from both stages; when D is Avg_Def, "
            "all Avg_Att and DistToAttCentroid controls are removed; when D is "
            "DistToDefCentroid, DistToAttCentroid and all Avg_Att controls are "
...
def build_reviewer_model_report(
    ...
    # Reviewer-facing omitted-variable sensitivity summary.
    if ovb_summary_df is not None and not ovb_summary_df.empty:
        ovb_add = ovb_summary_df.copy()
        duplicate_cols = [c for c in ovb_add.columns if c in out.columns and c not in keys]
        ovb_add = ovb_add.drop(columns=duplicate_cols)
        out = out.merge(ovb_add, on=keys, how="left")
    ...
    def evidence_label(r):
        ...
        if float(r.get("fold_sign_consistency", 0) or 0) < 0.60:
            return "CAUTION_fold_instability"
        if float(r.get("empirical_placebo_p", 1) or 1) > 0.10:
...
def build_reviewer_checklist(
    ...
    ovb_summary_df: pd.DataFrame,
    ovb_benchmark_df: pd.DataFrame,
    ovb_benchmark_scenario_df: pd.DataFrame,
) -> pd.DataFrame:
    no_leak = (
    ...
    items = [
        ...
        ("Second-stage uncertainty", "OLS with match-clustered standard errors", "05_main_oneD", "REPORTED" if report is not None and not report.empty else "MISSING"),
        ("Unobserved-confounding robustness values", "Point-estimate and alpha=0.05 robustness values plus treatment-outcome partial R2", "05_main_oneD / 29_ovb_summary", "REPORTED" if ovb_summary_df is not None and not ovb_summary_df.empty else "MISSING"),
        ("Observed-covariate confounding benchmarks", "Group partial R2 with treatment and outcome conditional on remaining observed controls", "30_ovb_observed_benchmarks", "REPORTED" if ovb_benchmark_df is not None and not ovb_benchmark_df.empty else "MISSING"),
        ("Bias-adjusted confounding scenarios", "0.5x, 1x and 2x observed-benchmark strength with adjusted effect, CI and p-value", "32_ovb_benchmark_scenarios", "REPORTED" if ovb_benchmark_scenario_df is not None and not ovb_benchmark_scenario_df.empty else "MISSING"),
...
def main():
    ...
    calibration_audits = []
    ovb_benchmark_all = []
    ovb_equal_scenario_all = []
    ovb_benchmark_scenario_all = []

    print("=" * 100)
    print("CARRY · SINGLE-KPI DML · L AND E' · RF+RF PRIMARY + TRAINING-FOLD UNDERSAMPLING SENSITIVITY")
    print(f"Input : {INPUT_PATH}")
    print(f"Output: {OUTPUT_XLSX}")
    print("=" * 100)

    excluded_dribble_rows = 0
    ...
    with StepTimer(runtime_log, "ALL", "ALL", "read_and_prepare"):
        ...
        if EXCLUDE_DRIBBLE_ROWS and TYPE_NAME_COL in df.columns:
            type_norm = (
                df[TYPE_NAME_COL]
                .astype("string")
                .str.strip()
            ...
            dribble_mask = type_norm.isin(EXCLUDED_TYPE_NAMES)
            excluded_dribble_rows = int(dribble_mask.sum())
            df = df.loc[~dribble_mask].copy()
        ...
        if not eprime_def_columns:
            ...
            raise ValueError(
                ...
                "请先在L时刻freeze_frame上围绕carry end_location计算E' KPI。"
            )
                ...
                try:
                    ...
                    with StepTimer(runtime_log, cid, stage, f"oneD_trims::{d_col}", n, n_matches):
                        ...
                        if not one.empty:
                            ...
                            if RUN_OVB_SENSITIVITY:
                                main_effect_rows = one.loc[
                                    np.isclose(one["trim_fraction_rule"], MAIN_TRIM_FRAC)
                                ].copy()
                                if not main_effect_rows.empty:
                                    with StepTimer(
                                        runtime_log, cid, stage, f"ovb_sensitivity::{d_col}",
                                        n, n_matches,
                                        note="observed-covariate partial-R2 benchmarks and bias-adjusted scenarios",
                                    ):
                                        benchmark = observed_covariate_benchmark_partial_r2(
                                            X_df=X_df, D=D[:, 0], y=y, block=block,
                                            cluster=cid, stage=stage, treatment=d_col,
                                        )
                                        if not benchmark.empty:
                                            ovb_benchmark_all.append(benchmark)
                                        equal_scen, benchmark_scen = build_ovb_scenario_tables(
                                            effect_row=main_effect_rows.iloc[0],
                                            benchmark_df=benchmark,
                                            cluster=cid, stage=stage, treatment=d_col,
                                        )
                                        if not equal_scen.empty:
                                            ovb_equal_scenario_all.append(equal_scen)
                                        if not benchmark_scen.empty:
                                            ovb_benchmark_scenario_all.append(benchmark_scen)

                    # ------------------------------------------------------
                    # Class-imbalance sensitivity: RF outcome model only.
                    # D|X predictions, test rows, calibration validation rows and
                    # the second-stage sample retain the original distribution.
                    # ------------------------------------------------------
                    if (
                        RUN_UNDERSAMPLING_SENSITIVITY and
                        stage in UNDERSAMPLING_ACTIVE_STAGES
                    ):
                        with StepTimer(
                            runtime_log, cid, stage, f"undersampling_sensitivity::{d_col}",
                            n, n_matches,
                            note=(
                            ...
                            us_cf = crossfit_undersampled_rf_y(
                                X=X, y=y, groups=groups, splits=splits,
                                seed=cluster_seed(cid, stage, 50000 + d_index * 19),
                            )

                        us_cal = us_cf.get("calibration_audit", pd.DataFrame()).copy()
                        if not us_cal.empty:
                        ...
                        us_sampling = us_cf.get("sampling_audit", pd.DataFrame()).copy()
                        ...
                        yhat_us = us_cf["yhat"]
                        y_res_us = y - yhat_us
                        # Reuse the primary RF D|X residuals to isolate the effect
                        # of outcome-class handling rather than changing both nuisances.
                        us_one = run_oneD_all_trims(
                            y_res_us, D_res, [d_col], groups, raw_sd_map,
                            cid, stage,
                            "rf_outcome_training_fold_undersampling_1to2_sensitivity",
                        )
                        if not us_one.empty:
                            us_one["n_controls"] = X.shape[1]
                            us_one["undersampling_target_ratio"] = UNDERSAMPLING_MINORITY_TO_MAJORITY_RATIO
                            us_one["treatment_nuisance_reused_from_primary"] = True
                            undersample_oneD_all.append(us_one)

                        us_raw_perf = outcome_performance_rows(
                            cid, stage, y, us_cf["yhat_raw"],
                            "RandomForestClassifier_training_fold_undersampled_raw",
                        )
                        us_raw_perf["treatment_specific_X"] = d_col
                        us_raw_perf["n_controls"] = X.shape[1]
                        us_raw_perf["undersampling_target_ratio"] = UNDERSAMPLING_MINORITY_TO_MAJORITY_RATIO
                        undersample_y_perf.append(us_raw_perf)

                        us_cal_perf = outcome_performance_rows(
                            cid, stage, y, yhat_us,
                            "RandomForestClassifier_training_fold_undersampled_NestedPlatt",
                        )
                        us_cal_perf["treatment_specific_X"] = d_col
                        us_cal_perf["n_controls"] = X.shape[1]
                        us_cal_perf["undersampling_target_ratio"] = UNDERSAMPLING_MINORITY_TO_MAJORITY_RATIO
    ...
    if not undersample_df.empty:
        main_mask_us = np.isclose(undersample_df["trim_fraction_rule"], MAIN_TRIM_FRAC)
        undersample_main_df = apply_fdr_columns(undersample_df.loc[main_mask_us].copy())
        undersample_df = pd.concat(
            [undersample_main_df, undersample_df.loc[~main_mask_us].copy()],
            ignore_index=True,
        )
    else:
    ...
    if not main_oneD_df.empty:
        ...
        main_oneD_df["candidate_for_discussion"] = (
            ...
            (~overlap_flag_series) &
            (fold_sign_series >= 0.60) &
            (placebo_p_series <= 0.10)
        )
    ...
    if not undersample_y_perf_df.empty:
        y_perf_df = pd.concat([y_perf_df, undersample_y_perf_df], ignore_index=True, sort=False)
    ...
    ovb_benchmark_df = (
        pd.concat(ovb_benchmark_all, ignore_index=True) if ovb_benchmark_all else pd.DataFrame()
    )
    ovb_equal_scenario_df = (
        pd.concat(ovb_equal_scenario_all, ignore_index=True) if ovb_equal_scenario_all else pd.DataFrame()
    )
    ovb_benchmark_scenario_df = (
        pd.concat(ovb_benchmark_scenario_all, ignore_index=True)
        if ovb_benchmark_scenario_all else pd.DataFrame()
    )
    ovb_summary_df = build_ovb_summary(
        main_df=main_oneD_df,
        benchmark_df=ovb_benchmark_df,
        benchmark_scenarios_df=ovb_benchmark_scenario_df,
    )
    ovb_contour_df = build_ovb_contour_grid(main_oneD_df)

    # Primary-vs-undersampled effect comparison at the prespecified main trim.
    if not main_oneD_df.empty and not undersample_main_df.empty:
        us_keep = undersample_main_df[[
            "cluster", "stage", "treatment", "theta_per_1sd", "se_per_1sd",
            "ci_low_per_1sd", "ci_high_per_1sd", "p_value", "q_global",
            "fdr_global_pass", "n_used", "n_matches",
        ]].rename(columns={
            "theta_per_1sd": "undersampled_theta_per_1sd",
            "se_per_1sd": "undersampled_se_per_1sd",
            "ci_low_per_1sd": "undersampled_ci_low_per_1sd",
            "ci_high_per_1sd": "undersampled_ci_high_per_1sd",
            "p_value": "undersampled_p_value",
            "q_global": "undersampled_q_global",
            "fdr_global_pass": "undersampled_fdr_global_pass",
            "n_used": "undersampled_n_used",
            "n_matches": "undersampled_n_matches",
        })
        orig_keep = main_oneD_df[[
            "cluster", "stage", "treatment", "theta_per_1sd", "se_per_1sd",
            "ci_low_per_1sd", "ci_high_per_1sd", "p_value", "q_global",
            "fdr_global_pass", "n_used", "n_matches",
        ]].rename(columns={
            "theta_per_1sd": "primary_theta_per_1sd",
            "se_per_1sd": "primary_se_per_1sd",
            "ci_low_per_1sd": "primary_ci_low_per_1sd",
            "ci_high_per_1sd": "primary_ci_high_per_1sd",
            "p_value": "primary_p_value",
            "q_global": "primary_q_global",
            "fdr_global_pass": "primary_fdr_global_pass",
            "n_used": "primary_n_used",
            "n_matches": "primary_n_matches",
        })
        undersample_compare_df = orig_keep.merge(
            us_keep, on=["cluster", "stage", "treatment"], how="inner"
        )
        ...
        undersample_compare_df["relative_absolute_effect_change"] = (
            ...
            / undersample_compare_df["primary_theta_per_1sd"].abs().replace(0, np.nan)
        )
        ...
        def imbalance_classification(row):
            ...
            if (
                np.isfinite(row["relative_absolute_effect_change"])
                and row["relative_absolute_effect_change"] > UNDERSAMPLING_MAGNITUDE_CHANGE_FLAG
            ):
        ...
        undersample_compare_df["imbalance_sensitivity_class"] = (
            undersample_compare_df.apply(imbalance_classification, axis=1)
        )
    ...
    reviewer_full_df = build_reviewer_model_report(
        ...
        ovb_summary_df=ovb_summary_df,
    )
    if not reviewer_full_df.empty and not undersample_compare_df.empty:
        add_cols = [
            "cluster", "stage", "treatment",
            "undersampled_theta_per_1sd", "undersampled_se_per_1sd",
            "undersampled_ci_low_per_1sd", "undersampled_ci_high_per_1sd",
            "undersampled_p_value", "undersampled_q_global",
            "undersampled_fdr_global_pass", "direction_same",
            "absolute_effect_change", "relative_absolute_effect_change",
        ...
        reviewer_full_df = reviewer_full_df.merge(
            ...
            on=["cluster", "stage", "treatment"], how="left",
        )
    ...
    reviewer_checklist_df = build_reviewer_checklist(
        ...
        ovb_summary_df=ovb_summary_df,
        ovb_benchmark_df=ovb_benchmark_df,
        ovb_benchmark_scenario_df=ovb_benchmark_scenario_df,
    )
    ...
    reviewer_checklist_df = pd.concat([
        ...
        pd.DataFrame([{
            ...
            "reported_evidence": (
                "RF outcome nuisance refitted after 1:2 majority undersampling inside "
                "each inner/outer training fold; calibration validation and outer test "
                "rows retain the original class distribution"
            ),
            "workbook_location": (
                "34_undersample_oneD / 35_undersample_compare / "
                "36_undersample_y_perf / 37_undersample_sampling / "
                "38_undersample_calibration"
            ),
            "status": (
                "REPORTED" if undersample_compare_df is not None
                and not undersample_compare_df.empty else "NOT_RUN"
            ),
        }])
    ], ignore_index=True)
    ...
    manifest = pd.DataFrame([{
        ...
        "excluded_type_name_rows": excluded_dribble_rows,
        ...
        "L_estimand": "one defending Carry L KPI conditional on reduced background plus permitted Att(L)/Att(E') KPIs; target-matched Att controls are removed from both stages; Avg and centroid attacking controls are reciprocally excluded; every Def KPI and every actual-E KPI is excluded from X",
        "Eprime_estimand": "one defending Carry E' KPI conditional on reduced background plus permitted Att(L)/Att(E') KPIs; target-matched Att controls are removed from both stages; Avg and centroid attacking controls are reciprocally excluded; every Def KPI and every actual-E KPI is excluded from X",
        ...
        "undersampling_sensitivity": (
            "RF outcome nuisance only; random majority undersampling inside every "
            "inner/outer training fold; inner validation and outer test rows unchanged"
        ...
        "undersampling_treatment_model_policy": "reuse primary RF D|X OOF residuals to isolate outcome-class handling",
        ...
        "second_stage": "OLS with match-clustered standard errors",
        "ovb_sensitivity": "robustness values + observed-covariate group partial-R2 benchmarks + benchmark-scaled bias-adjusted estimates" if RUN_OVB_SENSITIVITY else "not run",
        "ovb_alpha": OVB_ALPHA,
        "ovb_benchmark_multipliers": json.dumps(OVB_BENCHMARK_MULTIPLIERS),
        "ovb_equal_strength_r2_grid": json.dumps(OVB_EQUAL_STRENGTH_R2_GRID),
        "ovb_contour_r2_grid": json.dumps(OVB_CONTOUR_R2_GRID),
        "ovb_inference_caveat": "Cinelli-Hazlett partial-R2 formulas applied to cluster-robust coefficient/SE and match-level df; observed benchmark strengths use linear projections while main nuisances remain nonlinear and cross-fitted",
        "external_validation": "not included; requires a separate external dataset and frozen analysis protocol",
        "joint_DML": "not run in this single-KPI script",
        "nonlinear_joint_check": "not run in this single-KPI script",
        "interpretation_note": "Carry E' KPIs are measured around the corrected carry endpoint using the L-time freeze frame; every actual E-stage KPI is ignored.",
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        tables = OrderedDict([
            ...
            ("29_ovb_summary", ovb_summary_df),
            ("30_ovb_observed_benchmarks", ovb_benchmark_df),
            ("31_ovb_equal_strength", ovb_equal_scenario_df),
            ("32_ovb_benchmark_scenarios", ovb_benchmark_scenario_df),
            ("33_ovb_contour_grid", ovb_contour_df),
            ("34_undersample_oneD", undersample_df),
            ("35_undersample_compare", undersample_compare_df),
            ("36_undersample_y_perf", undersample_y_perf_df),
            ("37_undersample_sampling", undersample_sampling_audit_df),
            ("38_undersample_calibration", undersample_calibration_audit_df),
        ])

# -*- coding: utf-8 -*-
"""
Shot-time · DML-informed independent multi-KPI GA policy optimization
==================================================================
...
Workflow
--------
1. Read the completed Shot-time single-KPI and rotating joint-DML workbooks.
...
Examples
--------
Run the Shot-time policy analysis::
...
def load_candidate_pool(single_path: str, joint_path: str) -> pd.DataFrame:
    """
    Cross-check Shot-time against BOTH completed DML analyses.
...
def select_main_kpis(candidate_pool: pd.DataFrame) -> Tuple[Dict[int, List[str]], pd.DataFrame]:
    ...
    for cluster in sorted(pd.unique(candidate_pool["cluster"])):
        if cluster in FINAL_KPI_OVERRIDE:
            ...
            if noneligible:
                warnings.warn(
                    f"Exploratory Shot-time override in Cluster {cluster}: "
        ...
        if not chosen:
            raise RuntimeError(
                f"Cluster {cluster} has no KPI passing both Shot-time DML stages at "
...
def build_feature_columns(
    ...
    for col in SAFE_CATEGORICAL_CONTEXT_COLS:
        if col in df.columns and df[col].nunique(dropna=False) > 1:
            # sklearn 1.5's CompleteCaseTransformer can raise "boolean value of NA is
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        default=OUTPUT_XLSX,
        help="Output workbook for the Shot-time policy analysis.",
    )
...
def main():
    ...
    started = time.time()
    print("=" * 88)
    print("Shot-time independent DML-informed multi-KPI GA policy optimization")
    print("Stage          : L only")

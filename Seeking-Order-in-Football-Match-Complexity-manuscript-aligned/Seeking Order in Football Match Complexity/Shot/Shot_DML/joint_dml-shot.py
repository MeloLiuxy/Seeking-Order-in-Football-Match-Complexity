# -*- coding: utf-8 -*-
"""
Shot · Rotating joint and interaction DML for the shot-time configuration
========================================================================
...
through a three-mechanism joint interaction model at the shot-time configuration.
...
RANDOM_SEED = 42
N_SPLITS = 5
ACTIVE_STAGES = ("L",)  # Shot only has the shot-time configuration
...
def validate_primary_rf_alignment() -> None:
    ...
    if mismatches:
        raise ValueError(
            "Primary RF settings no longer match the final Shot-time-only single-KPI model: "
            ...
            if missing:
                target_map.append({
                    ...
                    "selection_basis": (
                        ...
                        "only jointly supported cluster-specific KPIs enter strategy optimization"
                    ),
...
def main():
    ...
    manifest = pd.DataFrame([{
        ...
        "L_estimand": "one defending Shot-time KPI conditional on reduced background plus permitted Att(L)/Att(E') KPIs; target-matched Att controls are removed from both stages; Avg and centroid attacking controls are reciprocally excluded; every Def KPI and every actual-E KPI is excluded from X",
...
def joint_main():
    ...
    print("=" * 110)
    print("SHOT · ROTATING JOINT DML · SHOT-TIME CONFIGURATION")
    print(f"Input : {INPUT_PATH}")
    print(f"Output: {OUTPUT_XLSX}")
    print("=" * 110)

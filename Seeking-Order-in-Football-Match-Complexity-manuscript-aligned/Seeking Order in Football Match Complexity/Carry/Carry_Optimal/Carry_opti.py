# -*- coding: utf-8 -*-
"""
Carry E' · DML-informed independent multi-KPI GA policy optimization
====================================================================
...
Version 5 retains the leakage-safe feature allow-list and integer decision constraints, and adds the reviewer-facing robustness analyses required for the independent policy stage: a context-only predictive baseline, alternative policy learners, both global and within-match conditional label-permutation diagnostics, explicit optimal-strategy value tables, and GA random-seed stability. It does not pass every
raw CSV column into the policy model, because outcome-derived and downstream event
fields can leak defensive success and create an impossible AUC of 1.0.

Workflow
--------
1. Read the completed rotating joint-DML workbook.
2. Build a cluster-specific candidate pool containing only KPIs that passed both
   the single-KPI and rotating joint-model robustness gates.
3. Select one KPI per prespecified tactical family for the main analysis:
      local advantage    : Adv_5 / Adv_10
      defensive distance : Avg_1/3/5_Def / DistToDefCentroid
      structure/shape    : Area_Def / Spr_Def
   The default rule is the largest absolute standardized single-KPI AME among candidates that retain joint support.
4. End the DML stage.
...
6. Optimize only failed held-out events. A genetic algorithm jointly changes the
   three selected KPIs to maximize out-of-fold model-predicted defensive-success
...
7. Compare the joint three-KPI action with the best one-KPI action and report
...
RAW_INPUT_PATH = r""
ROTATING_JOINT_RESULTS_XLSX = r""
OUTPUT_XLSX = r""

PIPELINE_VERSION = "independent_policy_v5_2_conditional_direction_fix_2026-07-29"
STAGE = "E'"
RANDOM_SEED = 42

# Main KPI selection. All passed KPIs form the candidate pool; the main policy
# changes exactly one KPI from each tactical family.
SELECTION_MODE = "largest_abs_single_AME_with_joint_support"
EXPECTED_FAMILIES = ("local_advantage", "distance_centroid", "structure_shape")
FINAL_KPI_OVERRIDE: Dict[int, Tuple[str, str, str]] = {
    # 0: ("Adv_10(E')", "Avg_5_Def(E')", "Area_Def(E')"),
    # 1: ("Adv_10(E')", "Avg_5_Def(E')", "Area_Def(E')"),
    # 2: ("Adv_10(E')", "Avg_5_Def(E')", "Area_Def(E')"),
}
...
# tactical decision point are allowed. The three selected defensive KPIs are
...
FORBIDDEN_FEATURE_TOKENS = (
    ...
    "carry_outcome", "end_reason", "defensive_success",
)
...
# The current Carry main policy selects Adv_10, and the rule also covers Adv_5.
...
BUDGET_SENSITIVITY_GRID = (1.0, 2.0, 4.0)
...
def direction_code_from_row(row: pd.Series) -> int:
    """
    Convert the joint-DML policy classification into a GA action constraint.

    - robust/context-dependent direction change -> two-sided action;
    - stable positive average effect            -> increase only;
    - stable negative average effect            -> decrease only.

    A single-to-joint sign reversal is never converted into a fixed direction.
    """
    label = str(row.get("policy_constraint_class", "")).upper()

    if (
        as_bool(row.get("robust_conditional_sign_change"), False)
        or "CONDITIONAL" in label
        or "TWO_SIDED" in label
        or "INTERACTION_AWARE" in label
    ):
        return 2
...
def load_candidate_pool(path: str) -> pd.DataFrame:
    """
    Build the E′ policy candidate pool.

    Main admission requires:
      1. strict single-KPI screen passed;
      2. rotating joint-model target FDR passed;
      3. the direction is usable for policy optimization, defined as either:
         a. stable single-to-joint direction agreement; or
         b. explicit interaction-aware / conditional-direction eligibility
            supplied by the joint-DML workbook.

    Training-fold undersampling remains a sensitivity analysis only.
    """
    df = pd.read_excel(path, sheet_name="02_target_AME_report")
    required = {
        "cluster", "stage", "target", "target_family", "estimate",
        "single_stage_pass", "fdr_global_pass",
        "single_to_joint_direction_same",
    }
    missing = sorted(required - set(df.columns))
    if missing:
        raise ValueError(f"Joint workbook is missing required columns: {missing}")

    out = df.copy()
    out["cluster"] = out["cluster"].map(normalize_cluster)
    out = out[out["stage"].astype(str).eq(STAGE)].copy()
    out["policy_family"] = out["target_family"].map(dml_family_to_policy_family)

    out["gate_single_stage_pass"] = out["single_stage_pass"].map(as_bool)
    out["gate_joint_fdr_pass"] = out["fdr_global_pass"].map(as_bool)
    out["gate_direction_same"] = out["single_to_joint_direction_same"].map(as_bool)

    if "robust_conditional_sign_change" in out.columns:
        out["gate_robust_conditional_change"] = (
            out["robust_conditional_sign_change"].map(as_bool)
        )
    else:
        out["gate_robust_conditional_change"] = False

    if "eligible_for_interaction_aware_optimizer" in out.columns:
        out["gate_interaction_aware_eligible"] = (
            out["eligible_for_interaction_aware_optimizer"].map(as_bool)
        )
    else:
        out["gate_interaction_aware_eligible"] = False

    policy_class = out.get(
        "policy_constraint_class",
        pd.Series("", index=out.index),
    ).astype(str).str.upper()
    out["gate_conditional_policy_class"] = (
        policy_class.str.contains("CONDITIONAL", na=False)
        | policy_class.str.contains("TWO_SIDED", na=False)
        | policy_class.str.contains("INTERACTION_AWARE", na=False)
    )

    # Backward-compatible fallback:
    # a robust conditional sign change or an explicit conditional policy class
    # is itself evidence that the row was designed for interaction-aware use.
    out["gate_interaction_aware_eligible"] = (
        out["gate_interaction_aware_eligible"]
        | out["gate_robust_conditional_change"]
        | out["gate_conditional_policy_class"]
    ...
    out["gate_direction_usable"] = (
        out["gate_direction_same"]
        | out["gate_interaction_aware_eligible"]
    )

    if "optimizer_eligible_after_undersampling" in out.columns:
        out["undersampling_optimizer_eligible"] = (
            out["optimizer_eligible_after_undersampling"].map(as_bool)
        )
    else:
        out["undersampling_optimizer_eligible"] = False

    if "imbalance_sensitivity_class" not in out.columns:
        out["imbalance_sensitivity_class"] = "NOT_REPORTED"
    ...
    out["passed_single_and_joint"] = (
        out["gate_single_stage_pass"]
        & out["gate_joint_fdr_pass"]
        & out["gate_direction_usable"]
    )

    out["policy_admission_type"] = np.select(
        [
            out["passed_single_and_joint"] & out["gate_direction_same"],
            out["passed_single_and_joint"] & ~out["gate_direction_same"]
            & out["gate_interaction_aware_eligible"],
        ],
        [
            "FIXED_DIRECTION",
            "CONDITIONAL_TWO_SIDED",
        ],
        default="NOT_ADMITTED",
    )

    out["main_admission_reason"] = np.where(
        out["passed_single_and_joint"],
        "PASS::" + out["policy_admission_type"],
        "FAIL::"
        + np.where(out["gate_single_stage_pass"], "", "single_stage;")
        + np.where(out["gate_joint_fdr_pass"], "", "joint_fdr;")
        + np.where(out["gate_direction_usable"], "", "direction_not_policy_usable;"),
    )
...
def select_main_kpis(candidate_pool: pd.DataFrame) -> Tuple[Dict[int, List[str]], pd.DataFrame]:
    ...
    for cluster in sorted(pd.unique(candidate_pool["cluster"])):
        if cluster in FINAL_KPI_OVERRIDE:
            ...
            if len(kpis) != 3:
                raise ValueError(f"FINAL_KPI_OVERRIDE[{cluster}] must contain three KPIs.")
            selected[int(cluster)] = kpis
            for family, kpi in zip(EXPECTED_FAMILIES, kpis):
                audit_rows.append({
                    "cluster": cluster,
        ...
        for family in EXPECTED_FAMILIES:
            ...
            if family_rows.empty:
                diagnostic_cols = [
                    "target", "policy_family",
                    "gate_single_stage_pass", "gate_joint_fdr_pass",
                    "gate_direction_same",
                    "gate_robust_conditional_change",
                    "gate_interaction_aware_eligible",
                    "gate_conditional_policy_class",
                    "gate_direction_usable",
                    "policy_constraint_class",
                    "direction_label",
                    "undersampling_optimizer_eligible",
                    "imbalance_sensitivity_class",
                    "main_admission_reason",
                ]
                available = candidate_pool[
                    (candidate_pool["cluster"] == cluster)
                    & (candidate_pool["policy_family"] == family)
                ].copy()
                available = available[
                    [c for c in diagnostic_cols if c in available.columns]
                ]
                raise RuntimeError(
                    f"Cluster {cluster} has no eligible KPI in family {family} "
                    "after fixed-direction OR interaction-aware admission.\n"
                    "Candidate audit:\n"
                    + available.to_string(index=False)
                )
            family_rows["abs_single_effect"] = pd.to_numeric(family_rows["single_theta_per_1sd"], errors="coerce").abs()
            sort_cols = ["abs_single_effect"]
            ascending = [False]
...
DEFENSIVE_EPRIME_PATTERN = re.compile(
    r"^(Adv_\d+|Avg_\d+_Def|Area_Def|Spr_Def|DistToDefCentroid)\(E'\)$",
    flags=re.IGNORECASE,
)


def _read_raw_table(path: str) -> pd.DataFrame:
    """Read CSV/TXT or auto-detect the main data sheet in an Excel workbook."""
    ...
    if suffix in {".xlsx", ".xls", ".xlsm"}:
        ...
        for sheet_name in excel.sheet_names:
            preview = pd.read_excel(input_path, sheet_name=sheet_name, nrows=5)
            cols = list(map(str, preview.columns))
            outcome = detect_column(cols, ["success_def", "outcome", "y"])
            cluster = detect_column(cols, ["cluster_id", "cluster", "Cluster"])
            match = detect_column(
                cols,
                ["__source_file__", "match_id", "match", "source_file", "file", "game_id"],
            )
            eprime_count = sum(str(c).endswith("(E')") for c in cols)
            score = (
                10 * int(outcome is not None)
                + 10 * int(cluster is not None)
                + 10 * int(match is not None)
                + eprime_count
            )
        ...
        if not candidates:
            raise ValueError(
                "No Excel worksheet contains outcome, cluster and match columns. "
                f"Sheets checked: {excel.sheet_names}"
            )

        candidates.sort(reverse=True)
        selected_sheet = candidates[0][1]
        print(f"[RAW] Excel worksheet selected: {selected_sheet}")
        return pd.read_excel(input_path, sheet_name=selected_sheet)

    raise ValueError(
        f"Unsupported raw input extension: {suffix}. "
        "Supported: .csv, .txt, .xlsx, .xls, .xlsm"
    )
...
def _strip_stage_suffix(name: str) -> str:
    text = str(name).strip()
    return re.sub(r"\((?:L|E')\)$", "", text, flags=re.IGNORECASE)


def _is_safe_attacking_kpi(name: str) -> bool:
    text = str(name).strip()
    if not re.search(r"\((?:L|E')\)$", text, flags=re.IGNORECASE):
        return False
...
def build_feature_columns(
    ...
    if missing:
        raise ValueError(
            ...
            "Check that the raw CSV is the full Carry E' table."
        )
...
def make_preprocessor(
    ...
    categorical_pipeline = Pipeline([
        ("imputer", CompleteCaseTransformer(strategy="most_frequent")),
...
def make_estimator(learner_name: str, seed: int):
...
def fit_policy_model(
    ...
    if n_inner >= 2 and len(np.unique(y)) == 2:
        ...
        for fold, (tr, va) in enumerate(inner.split(train_df, y, groups)):
            ...
            oof_raw[va] = predict_raw(pre, est, train_df.iloc[va], feature_columns)
...
def fit_support_model(values: np.ndarray, quantiles: Tuple[float, float]) -> SupportModel:
    ...
    if values.ndim != 2 or values.shape[1] != 3:
        raise ValueError("Main policy support requires n x 3 selected KPI values.")
    if len(values) < 30:
    ...
    z = (filled - mean) / sd
    cov = np.cov(z, rowvar=False) + SUPPORT_RIDGE * np.eye(3)
...
def evaluate_population(
    ...
    batch, pop_size, _ = population_raw.shape
    repeated = np.repeat(base_encoded, pop_size, axis=0)
    flat_raw = population_raw.reshape(batch * pop_size, 3)
...
def vectorized_ga(
    ...
    batch = len(observed)
    rng = np.random.RandomState(seed)
    population = lower[:, None, :] + rng.rand(batch, GA_POPULATION, 3) * (
        upper - lower
    )[:, None, :]
    ...
    for generation in range(GA_GENERATIONS):
        ...
        gene_mutation = rng.rand(batch, n_children, 3) < (1.0 / 3.0)
    ...
    for fold, (train_idx, eval_idx) in enumerate(splitter.split(cluster_df, y_all, groups_all)):
        ...
        if RUN_CONTEXT_ONLY_BASELINE:
            ...
            baseline_oof[eval_idx] = baseline_model.predict_encoded(
                baseline_model.transform(evaluation)
            )
        ...
        for alt_idx, learner_name in enumerate(alternative_names):
            ...
            alt_prob = alt_model.predict_encoded(alt_model.transform(evaluation))
        ...
        if RUN_GLOBAL_LABEL_PERMUTATION:
            for rep in range(GLOBAL_LABEL_PERMUTATION_REPS):
                ...
                perm_pre, perm_est = fit_raw_probability_model(
                    train, perm_y, feature_columns, numeric_columns,
                    categorical_columns,
                    seed=cluster_seed(cluster, 70000 + rep * 100 + fold),
                    undersample=False, learner_name=PRIMARY_POLICY_LEARNER,
                )
                global_permutation_oof[rep][eval_idx] = predict_raw(
                    perm_pre, perm_est, evaluation, feature_columns
                )
        ...
        if RUN_WITHIN_MATCH_CONDITIONAL_PERMUTATION:
            for rep in range(WITHIN_MATCH_CONDITIONAL_PERMUTATION_REPS):
                ...
                conditional_permutation_oof[rep][eval_idx] = predict_raw(
                    perm_pre, perm_est, evaluation, feature_columns
                )
...
def reviewer_checklist() -> pd.DataFrame:
    return pd.DataFrame([
        ["DML and optimization separated", True, "No DML model is fitted in this script."],
        ["Candidate KPIs passed single and joint DML", True, "Read from 02_target_AME_report and gated before selection."],
        ...
        ["Context-only predictive baseline", RUN_CONTEXT_ONLY_BASELINE, "The same grouped OOF learner is refit without the three actionable KPIs."],
...
def main():
    ...
    started = time.time()
    print("=" * 88)
    print("Carry independent DML-informed multi-KPI GA policy optimization (final robustness v5)")
    print(f"Raw input      : {args.raw}")
    print(f"DML results    : {args.joint_results}")
    print(f"Output         : {args.output}")
    print("No DML model is refitted in this script.")
    print(f"Available policy learners: {' | '.join(available_policy_learners())}")
    print("=" * 88)

    candidate_pool = load_candidate_pool(args.joint_results)

    print("Main KPI admission audit by cluster/family:")
    audit_summary = (
        candidate_pool.groupby(["cluster", "policy_family"], dropna=False)
        .agg(
            n_candidates=("target", "size"),
            n_main_eligible=("passed_single_and_joint", "sum"),
            n_fixed_direction=(
                "policy_admission_type",
                lambda s: int((s == "FIXED_DIRECTION").sum()),
            ),
            n_conditional_two_sided=(
                "policy_admission_type",
                lambda s: int((s == "CONDITIONAL_TWO_SIDED").sum()),
            ),
            n_undersampling_eligible=(
                "undersampling_optimizer_eligible",
                "sum",
            ),
        )
    ...
    for cluster, kpis in selected.items():
        for family, kpi in zip(EXPECTED_FAMILIES, kpis):
            ...
            selected_rows.append({
                ...
                "policy_family": family,
                "selected_kpi": kpi,
                "joint_AME_used_for_selection_only": row.get("estimate"),
    ...
    sheets = {
        "00_reviewer_checklist": reviewer_checklist(),

# -*- coding: utf-8 -*-
"""Construct manuscript KPIs for the carry-endpoint-referenced configuration.

The internal suffix `(E')` is retained for compatibility. The carry endpoint is
used only as the focal coordinate; all player-location information comes from the
carry-initiation snapshot.
"""
...
def main() -> None:
    ...
    for column in ("end_location_x", "end_location_y"):
        if column not in df:
            raise ValueError(f"Missing required carry endpoint coordinate: {column}")
        df[column] = to_num(df[column])
    if "freeze_frame" not in df:
        raise ValueError("Missing carry-initiation freeze_frame/event-time player-location field.")
    endpoint_rows = []
    for row in df.itertuples(index=False):
        endpoint = (getattr(row, "end_location_x"), getattr(row, "end_location_y"))
        endpoint_rows.append(compute_metrics(endpoint, getattr(row, "freeze_frame")))
    endpoint_df = pd.DataFrame(endpoint_rows).add_suffix("(E')")
    output = pd.concat([df.reset_index(drop=True), endpoint_df], axis=1)
    ...
    with pd.ExcelWriter(OUTPUT_XLS, engine="xlsxwriter") as writer:
        output.to_excel(writer, index=False, sheet_name="carries_with_metrics")
    print(f"[OK] Carry-endpoint-referenced KPIs written to {OUTPUT_XLS}")
